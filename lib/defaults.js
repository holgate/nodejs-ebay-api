  ...
  switch (options.serviceName) {
    case 'Finding':
      return {
        ...
        'X-EBAY-SOA-GLOBAL-ID': options.globalId ? options.globalId : (options.country ? countryHelper.getSiteId(options.country) : 'EBAY-US'),
    ...
    case 'Product':
      return {
        ...
        'GLOBAL-ID': options.globalId ? options.globalId : (options.country ? countryHelper.getSiteId(options.country) : 'EBAY-US'),
    ...
    case 'Shopping':
      return {
        ...
        'X-EBAY-API-SITE-ID': options.siteId ? options.siteId : (options.country ? countryHelper.getSiteCode(options.country) : '0'), //DEFAULT TO US (0)
        'X-EBAY-API-REQUEST-ENCODING': 'XML',
        'X-EBAY-API-RESPONSE-ENCODING': 'XML',
        'Content-Type': 'text/xml',
        //'Accept': 'text/xml'
        //Added by Duna to support affiliate link///
        'X-EBAY-API-TRACKING-ID': options.affiliate.trackingId,
        'X-EBAY-API-TRACKING-PARTNER-CODE': options.affiliate.networkId,
        'X-EBAY-API-AFFILIATE-USER-ID': options.affiliate.customId,
         ///End Added by Duna///////////
      };
    ...
    case 'Trading':
      return {
        ...
        'X-EBAY-API-SITEID': options.siteId ? options.siteId : (options.country ? countryHelper.getSiteCode(options.country) : '0'), //DEFAULT TO US (0)
